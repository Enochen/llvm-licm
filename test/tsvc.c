// Thanks @matth2k https://github.com/matth2k/cs6120-exercises/blob/main/l8/testcases/tsvc.c
// and thanks as well to TSVC authors https://dl.acm.org/doi/10.5555/62972.62987 

// import string and memalign modified for mac native clang compilation and execution
...
//  *ip = (int *)memalign(ARRAY_ALIGNMENT, LEN_1D * sizeof(real_t));
  ...
  void *xx_tmp = NULL;
  if (posix_memalign(&xx_tmp, ARRAY_ALIGNMENT, LEN_1D * sizeof(real_t)) != 0) {
    fprintf(stderr, "Memory allocation failed using posix_memalign for xx.\n");
    exit(EXIT_FAILURE);
  }
  xx = (real_t *)xx_tmp;

  void *ip_tmp = NULL;
  if (posix_memalign(&ip_tmp, ARRAY_ALIGNMENT, LEN_1D * sizeof(real_t)) != 0) {
    fprintf(stderr, "Memory allocation failed using posix_memalign for ip.\n");
    exit(EXIT_FAILURE);
  }
  *ip = (int *)ip_tmp;
...
int dummy(float a[LEN_1D], float b[LEN_1D], float c[LEN_1D], float d[LEN_1D],
...
int main(int argc, char **argv) {
  ...
  time_function(&s4112, &(struct {
    int *a;
    real_t b;
  }){ip, s1});
  time_function(&s4113, ip);
  time_function(&s4114, &(struct {
    int *a;
    int b;
  }){ip, n1});
  time_function(&s4115, ip);
  time_function(&s4116, &(struct {
    int *a;
    int b;
    int c;
  }){ip, LEN_2D / 2, n1});
